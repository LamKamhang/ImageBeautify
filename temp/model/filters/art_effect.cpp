...
static void ArtEffect::_emboss(const cv::Mat &src, cv::Mat &dst)
{
...
static void ArtEffect::_sculpture(const cv::Mat &src, cv::Mat &dst)
{
	Mat img(src.size(),CV_8UC3);
	for (int y=1; y<src.rows-1; y++)
	{
...
static void ArtEffect::_dilate(const cv::Mat &src, cv::Mat &dst, int shape = MORPH_RECT, int size = 5)
{
	dilate(src, dst, getStructuringElement(shape, Size(size, size)));
}

static void ArtEffect::_erode(const cv::Mat &src, cv::Mat &dst, int shape = MORPH_RECT, int size = 5)
{
