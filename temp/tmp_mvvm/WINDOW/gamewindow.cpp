...
{
    _ptrCommand->SetParameter(std::make_shared<IntParameters>(2));
    _ptrCommand->Exec();
}

void gamewindow::on_firstButton_clicked()
{
    _ptrCommand->SetParameter(std::make_shared<IntParameters>(1));
    _ptrCommand->Exec();
}
...
//    qDebug()<<x<<y;
    if(x>=40&&y>=40){
        ...
        if(row>row1||col>col1){
            ...
            _ptrMouseMoveCommand->SetParameter(std::make_shared<StringParameters>(a));
