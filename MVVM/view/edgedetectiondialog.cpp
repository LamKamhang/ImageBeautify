...
void EdgeDetectionDialog::apply(){
    ...
    obj.insert("type", std::make_shared<IntParameters>(type));
    if(type == 2){
        int lo, hi;
        ...
        obj.insert("threshold", std::make_shared<IntParameters>(th));
    }
    emit sendApplyEdgeDetection(std::make_shared<JsonParameters>(obj));
    close();
}
