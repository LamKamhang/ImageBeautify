...
void ScaleDialog::apply(){
    ...
    obj.insert("algo", std::make_shared<IntParameters>(btnGroup->checkedId()));
    obj.insert("scale", std::make_shared<IntParameters>(scaleFactorEdit->text().toInt()));
    obj.insert("rotation", std::make_shared<IntParameters>(rotationDegreeEdit->text().toInt()));
    emit sendApplyScale(std::make_shared<JsonParameters>(obj));
    close();
}
